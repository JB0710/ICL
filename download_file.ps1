...
while ($true) {
  try {
      ...
      $connection = Test-NetConnection -ComputerName "8.8.8.8" -Port 53 -WarningAction SilentlyContinue -InformationLevel Quiet -ErrorAction Stop
  ...
  # If no connection
  Write-Host "[$(Get-Date -Format 'HH:mm:ss')] No internet connection. Retrying in 5 seconds..." -ForegroundColor Yellow
